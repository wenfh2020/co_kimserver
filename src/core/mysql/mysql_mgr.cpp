...
MysqlMgr::get_co_mgr_data(const std::string& node) {
    ...
    co_create(&(md->dist_co), nullptr, [this, md](void*) { on_dist_task(md); });
            ...
            for (auto cd : md->free_conns) {
                ...
                if (cd->c != nullptr && now > (cd->active_time + CONN_TIME_OUT)) {
                    LOG_INFO("recover dbi conn, node: %s, coroutines cnt: %lu, co: %p, conn: %p",
                             cd->dbi->node.c_str(), md->conn_cnt, cd->conn_co, cd->c.get());
                    cd->c->close();
                    cd->c = nullptr;
                }
            }
...
void MysqlMgr::destory() {
    /* 协程须要优雅退出，否则直接删除会有问题的，
     * 使用了智能指针以后，任务能正常释放，其它有些结构指针不会释放，
     * 内存泄漏应该不多，不妨碍使用，有待后续优化吧。 */
    return;
    ...
    for (auto it : m_coroutines) {
        ...
        if (!md->busy_conns.empty()) {
            md->free_conns.splice(md->free_conns.begin(), md->busy_conns);
        }
        for (auto& cd : md->free_conns) {
            co_reset(cd->conn_co);
            co_release(cd->conn_co);
            co_cond_free(cd->conn_cond);
        }
    }
    m_dbs.clear();
    m_coroutines.clear();
